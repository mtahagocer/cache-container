import { IObjectCompareLRUCache, ObjectCompareLRUCache } from "./ObjectCompareLRUCache"
...
export interface IObjectCompareLRUApiCache<P extends Record<string, unknown>, R> {
    (props: P): Promise<R>,
...
): IObjectCompareLRUApiCache<P, R> => {

    const _cache = new ObjectCompareLRUCache<P, R>(cacheConfig);

    const cachedApiRequest = (props: P) => apiRequest(props)
        .then((res) => {
