...
export class ObjectCompareLRUCache<K extends Record<string, unknown>, V>
  implements IObjectCompareLRUCache {
  public size: number;
  ...
  set(key: K, value: V) {
    ...
    // refresh
    if (item)
      this.cache.splice(this.cache.indexOf(item as ICacheValue<K, V>), 1);
    // evict oldest
